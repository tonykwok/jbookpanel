...
implements MouseInputListener, ActionListener {
    ...
    protected void paintPageNumber(final Graphics g, final int index, final int width, final int height) {
        g.setFont(new Font("Arial", Font.BOLD, 11));
        final int w = (int) g.getFontMetrics().getStringBounds(String.valueOf(index), g).getWidth();
        final int x = (index % 2 == 0)? width - 8 - w : 8;
    ...
    public void mouseMoved(final MouseEvent e) {
        ...
        if (isMouseInRegion(e)) {
            final int xOffset = (leftPageTurn)? 10 : -10;
            final Point p = new Point(e.getX() + xOffset, e.getY() - 10);
            calculate(p);
    ...
    protected boolean isMouseInRegion(final MouseEvent e) {
        final int x = e.getX();
        final int y = e.getY();

        final int maxX = bookBounds.x + bookBounds.width;
        final int minX = maxX - cornerRegionSize;
        final int maxY = bookBounds.y + bookBounds.height;
        final int minY = maxY - cornerRegionSize;

        final int minX2 = bookBounds.x;
        final int maxX2 = minX2 + cornerRegionSize;
    ...
    protected boolean isMouseInBook(final MouseEvent e) {
    ...
    protected Image loadPage(final int index) {
        return new ImageIcon(JBookPanel.class.getResource(pageLocation + pageName
    ...
    public void setMargins(final int top, final int left) {
        bookBounds.x = left;
        bookBounds.y = top;
    ...
    public void setSoftClipping(final boolean softClipping) {
